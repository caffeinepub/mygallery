...
export default function HomePage() {
  ...
  const mainContent = useMemo(() => {
    ...
    if (isAuthenticated && isActorError && error) {
      return <ActorInitErrorState error={error} onRetry={retry} onLogout={signOut} />;
    }

    // Show loading state during initialization
    if (isInitializing || (isAuthenticated && status === 'initializing')) {
      return (
        <div className="flex min-h-screen flex-col">
          <Header />
  ...
  }, [isAuthenticated, isInitializing, status, isActorReady, isActorError, error, retry, signOut, selectedFolder, isFoldersDialogOpen]);
