...
import { type ExternalBlob, SortDirection } from "@/backend";
...
export function useGetFilesInFolder(folderId: bigint | null) {
  ...
  return useQuery<FileMetadata[]>({
    ...
    queryFn: async () => {
      ...
      const result = await actor.getFilesInFolder(
        folderId,
        BigInt(0),
        BigInt(1000),
      );
...
export function useDeleteFolder() {
  ...
  return useMutation({
    ...
    onMutate: async (folderId) => {
      ...
      const previousFolders = queryClient.getQueryData<Folder[]>(["folders"]);
      const previousFolderFiles = queryClient.getQueryData<FileMetadata[]>([
        "files",
        "folder",
        folderId.toString(),
      ]);
    ...
    onError: (err, folderId, context) => {
      ...
      if (context?.previousFolderFiles) {
        queryClient.setQueryData(
          ["files", "folder", folderId.toString()],
          context.previousFolderFiles,
        );
      }
...
export function useMoveFilesToFolder() {
  ...
  return useMutation({
    mutationFn: async ({
      fileIds,
      folderId,
    }: { fileIds: string[]; folderId: bigint }) => {
      if (!actor || status !== "ready") {
        throw createActorNotReadyError();
      }
    ...
    onMutate: async ({ fileIds, folderId }) => {
      ...
      const previousMainFiles = queryClient.getQueryData<FileMetadata[]>([
        "files",
        "not-in-folder",
      ]);
      const previousTargetFiles = queryClient.getQueryData<FileMetadata[]>([
        "files",
        "folder",
        folderId.toString(),
      ]);
      ...
      const filesToMove =
        previousMainFiles?.filter((f) => fileIds.includes(f.id)) ?? [];

      queryClient.setQueryData<FileMetadata[]>(
        ["files", "not-in-folder"],
      ...
      queryClient.setQueryData<FileMetadata[]>(
        ["files", "folder", folderId.toString()],
        (old = []) => [...filesToMove.map((f) => ({ ...f, folderId })), ...old],
      );
    ...
    onError: (err, { folderId }, context) => {
      ...
      if (context?.previousMainFiles) {
        queryClient.setQueryData(
          ["files", "not-in-folder"],
          context.previousMainFiles,
        );
      }
      if (context?.previousTargetFiles) {
        queryClient.setQueryData(
          ["files", "folder", folderId.toString()],
          context.previousTargetFiles,
        );
      }
...
export function useMoveFilesToMission() {
  ...
  return useMutation({
    mutationFn: async ({
      fileIds,
      missionId,
    }: { fileIds: string[]; missionId: bigint }) => {
      if (!actor || status !== "ready") {
        throw createActorNotReadyError();
      }
    ...
    onMutate: async ({ fileIds, missionId }) => {
      ...
      const previousMissionFiles = queryClient.getQueryData<FileMetadata[]>([
        "files",
        "mission",
        missionId.toString(),
      ]);

      const allFolderQueries = queryClient.getQueriesData<FileMetadata[]>({
      ...
      queryClient.setQueryData<FileMetadata[]>(
        ["files", "mission", missionId.toString()],
        (old = []) => [
          ...filesToMove.map((f) => ({ ...f, missionId, folderId: undefined })),
          ...(old ?? []),
        ],
      );
...
export function useBatchRemoveFromFolder() {
  ...
  return useMutation({
    mutationFn: async ({
      fileIds,
      sourceFolderId,
    }: { fileIds: string[]; sourceFolderId?: bigint }) => {
      if (!actor || status !== "ready") {
        throw createActorNotReadyError();
      }

      return timeOperation(
        "batchRemoveFromFolder",
        async () => {
          // Convert string IDs to bigint for backend
          const bigintIds = fileIds.map((id) => BigInt(id));
    ...
    onMutate: async ({ fileIds, sourceFolderId }) => {
      ...
      const previousFolderFiles = sourceFolderId
        ? queryClient.getQueryData<FileMetadata[]>([
            "files",
            "folder",
            sourceFolderId.toString(),
          ])
        : undefined;

      // Files from folder move to main collection
      const filesToMove =
        previousFolderFiles?.filter((f) => fileIds.includes(f.id)) ?? [];

      queryClient.setQueryData<FileMetadata[]>(
        ["files", "not-in-folder"],
        (old = []) => [
          ...filesToMove.map((f) => ({ ...f, folderId: undefined })),
          ...old,
        ],
      );
      ...
      if (sourceFolderId) {
        queryClient.setQueryData<FileMetadata[]>(
          ["files", "folder", sourceFolderId.toString()],
          (old = []) => old.filter((f) => !fileIds.includes(f.id)),
        );
      }
    ...
    onError: (err, { sourceFolderId }, context) => {
      ...
      throw new Error(
        `Failed to return files to main collection: ${errorMessage}`,
      );
    },
    onSuccess: async (_, { sourceFolderId }) => {
      ...
      if (sourceFolderId) {
        await queryClient.invalidateQueries({
          queryKey: ["files", "folder", sourceFolderId.toString()],
          exact: true,
        });
      }
    },
...
export function useDeleteFiles() {
  ...
  return useMutation({
    ...
    onMutate: async (fileIds) => {
      ...
      const allMissionQueries = queryClient.getQueriesData<FileMetadata[]>({
        queryKey: ["files", "mission"],
      });
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return uploadLimiter(async () => {
        perfDiag.startTiming(`upload-${name}`, "Upload file", {
          fileName: name,
          size: size.toString(),
        });
...
export function useCreateLink() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return uploadLimiter(async () => {
        const response = await actor.createLink(
          name,
          url,
          folderId ?? null,
          missionId ?? null,
