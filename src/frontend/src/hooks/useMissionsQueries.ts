...
export function useCreateMission() {
  ...
  return useMutation({
    mutationFn: async ({ title, tasks }: { title: string; tasks: Task[] }) => {
      ...
      logMissionMutationPhase("createMission", "start", {
        title,
        taskCount: tasks.length,
      });
      const missionId = await actor.createMission(title, tasks);
      logMissionMutationPhase("createMission", "success", {
    ...
    onError: (error) => {
      logMissionMutationPhase("createMission", "error", {
        error: String(error),
      });
...
export function useUpdateMission() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      title,
      tasks,
    }: { missionId: bigint; title: string; tasks: Task[] }) => {
      if (!actor || status !== "ready") {
        throw createActorNotReadyError();
      }

      logMissionMutationPhase("updateMission", "start", {
        missionId: missionId.toString(),
        title,
        taskCount: tasks.length,
      });
      await actor.updateMission(missionId, title, tasks);
      logMissionMutationPhase("updateMission", "success", {
        missionId: missionId.toString(),
      });
    ...
    onMutate: async ({ missionId, title, tasks }) => {
      ...
      const previousMission = queryClient.getQueryData<Mission | null>([
        "missions",
        "detail",
        missionId.toString(),
      ]);
      ...
      if (previousMission) {
        queryClient.setQueryData<Mission | null>(
          ["missions", "detail", missionId.toString()],
          {
            ...previousMission,
            title,
            tasks,
    ...
    onError: (err, { missionId }, context) => {
      logMissionMutationPhase("updateMission", "error", {
        missionId: missionId.toString(),
        error: String(err),
      });
...
export function useAddTaskToMission() {
  ...
  return useMutation({
    mutationFn: async ({
      missionId,
      task,
    }: { missionId: bigint; task: string }) => {
      if (!actor || status !== "ready") {
        throw createActorNotReadyError();
      }

      logMissionMutationPhase("addTaskToMission", "start", {
        missionId: missionId.toString(),
        task,
      });
      const newTask = await actor.addTaskToMission(missionId, task);
      logMissionMutationPhase("addTaskToMission", "success", {
        missionId: missionId.toString(),
        taskId: newTask.taskId.toString(),
      });
    ...
    onError: (err, { missionId }, context) => {
      logMissionMutationPhase("addTaskToMission", "error", {
        missionId: missionId.toString(),
        error: String(err),
      });
      const errorMessage = getActorErrorMessage(err);
    ...
    onSuccess: async ({ missionId, newTask }) => {
      ...
      const currentMission = queryClient.getQueryData<Mission | null>([
        "missions",
        "detail",
        missionId.toString(),
      ]);
      ...
      if (currentMission) {
        ...
        const tasksWithoutOptimistic = currentMission.tasks.filter(
          (t) => t.taskId < BigInt(Date.now() * 1000),
        );
        queryClient.setQueryData<Mission | null>(
          ["missions", "detail", missionId.toString()],
          {
...
export function useToggleTaskCompletion() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      taskId,
      completed,
    }: { missionId: bigint; taskId: bigint; completed: boolean }) => {
      if (!actor || status !== "ready") {
        throw createActorNotReadyError();
      }

      logMissionMutationPhase("toggleTaskCompletion", "start", {
        missionId: missionId.toString(),
      ...
      logMissionMutationPhase("toggleTaskCompletion", "success", {
        missionId: missionId.toString(),
        taskId: taskId.toString(),
      });
    ...
    onMutate: async ({ missionId, taskId, completed }) => {
      ...
      if (previousMission) {
        ...
        const missionsList = queryClient.getQueryData<Mission[]>([
          "missions",
          "list",
        ]);
        if (missionsList) {
    ...
    onError: (err, variables, context) => {
      logMissionMutationPhase("toggleTaskCompletion", "error", {
        missionId: variables.missionId.toString(),
        error: String(err),
      });
      const errorMessage = getActorErrorMessage(err);
      ...
      if (context?.previousMission) {
        const currentMission = queryClient.getQueryData<Mission | null>([
          "missions",
          "detail",
          context.missionId.toString(),
        ]);
        const currentTask = currentMission?.tasks.find(
          (t) => t.taskId === context.taskId,
        );
        ...
        if (currentTask && currentTask.completed === context.completed) {
          queryClient.setQueryData(
            ["missions", "detail", context.missionId.toString()],
            context.previousMission,
          );
...
export function useDeleteMission() {
  ...
  return useMutation({
    mutationFn: async (missionId: bigint) => {
      ...
      logMissionMutationPhase("deleteMission", "start", {
        missionId: missionId.toString(),
      });
      await actor.deleteMission(missionId);
      logMissionMutationPhase("deleteMission", "success", {
        missionId: missionId.toString(),
      });
    ...
    onError: (err, missionId, context) => {
      logMissionMutationPhase("deleteMission", "error", {
        missionId: missionId.toString(),
        error: String(err),
      });
      const errorMessage = getActorErrorMessage(err);
