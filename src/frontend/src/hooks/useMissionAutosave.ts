...
export function useMissionAutosave({
  ...
  const pendingUpdateRef = useRef<{ title: string; tasks: Task[] } | null>(
    null,
  );
  ...
  const lastHydratedStateRef = useRef<{
    title: string;
    tasks: Task[];
    missionId: string;
  } | null>(null);
  ...
  const performSave = useCallback(
    async (saveTitle: string, saveTasks: Task[]) => {
  ...
  const markAsHydrated = useCallback(
    (initialTitle: string, initialTasks: Task[]) => {
      // Store a deep copy to prevent mutation issues
      lastHydratedStateRef.current = {
        title: initialTitle,
  ...
  const flushPendingSave = useCallback(
    async (currentTitle: string, currentTasks: Task[]) => {
