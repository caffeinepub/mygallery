...
import { FileMetadata, Note } from '../backend';
...
export default function CollectionsFullScreenView({
  ...
  const { data: filesData, isLoading: filesLoading } = useQuery<{
    ...
    queryFn: async () => {
      ...
      return actor.getPaginatedFiles(SortDirection.desc, BigInt(0), BigInt(500));
    },
  ...
  const { data: notesData, isLoading: notesLoading } = useQuery<{
    ...
    queryFn: async () => {
      ...
      return actor.getPaginatedNotes(SortDirection.desc, BigInt(0), BigInt(500));
    },
    enabled: !!actor && !actorFetching,
    staleTime: 2 * 60 * 1000,
  });
  ...
  const handleItemLongPress = useCallback(
    ...
    [selectionMode, enterSelectionMode, toggleFileSelection, toggleNoteSelection]
  );
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Shared from Collections', text: names });
      }
    } catch {
  ...
  const selectedNoteIdsArray: bigint[] = Array.from(selectedNoteIds).map((id) => BigInt(id));
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
          ...
          >
            ...
            <p className="text-center" style={{ fontSize: 14, color: subTextColor }}>
